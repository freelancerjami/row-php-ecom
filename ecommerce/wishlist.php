	...
	<section id="content">
		<div class="content-blog content-account">
			...
			<table class="cart-table account-table table table-bordered">
				...
				<tbody>
				<?php
					$wishSql="SELECT p.name,p.price,p.id AS pid,w.id AS id,w.timestamp FROM wishlist w JOIN products p WHERE w.pid=p.id AND w.uid=$uid ORDER BY w.timestamp DESC ";
					$wishQuery=mysqli_query($con,$wishSql);
					$i=1;
					while($wishData=mysqli_fetch_assoc($wishQuery)){
				...
				?>	
					<tr>
						...
						<td>
							<a href="singel.php?id=<?php echo $wishData['pid']; ?>"><?= $wishData['name']; ?></a>
						</td>
						<td>
							$<?= $wishData['price']; ?>
						</td>
						...
						<td><a href="delwishlist.php?id=<?php echo $wishData['id']; ?>">Delete</a></td>
					</tr>
				<?php $i++;} ?>
				</tbody>
			</table>		
